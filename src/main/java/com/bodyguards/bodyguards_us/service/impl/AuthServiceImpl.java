...
public class AuthServiceImpl implements AuthService {
    ...
    public String forgotPassword(ForgotPasswordRequest request) {
        ...
        int numberOfRequestToday = passwordResetRequestRepository
                .countByUserIdAndRequestTimeBetween(user.getIdUser(), startDate, endDate);
    ...
    public String updatePassword(UpdatePasswordRequest request) {
        ...
        PasswordResetToken passwordResetToken = passwordResetTokenRepository
                .findByToken(token)
                .orElseThrow(
                        () -> new ApiException(ErrorCode.INVALID_RESET_PASSWORD_TOKEN)
                );

        if (LocalDateTime.now().isAfter(passwordResetToken.getExpiredAt()) ||
